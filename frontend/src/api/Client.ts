import { IApiResponse, type IUser } from "../types"
    ...
    private async request<T>(endpoint: string,
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new ApiError(
                    response.status, errorData.error || `HTTP  ${response.status}: ${response.statusText}`
                )
...
export const apiClient = new ApiClient(`${import.meta.env.VITE_URL}/api`)